...
#for Task 3.3
[x1_t,x2_t,x3_t,y_training] = splitData(data,0,10000,'Task3.3')
x1training = bias(x1_t)
x2training = bias(x2_t)
x3training = bias(x3_t)

[x1_v, x2_v, x3_v, y_validation] = splitData(data,10000,11000,'Task3.3')
x1validation = bias(x1_v)
x2validation = bias(x2_v)
x3validation = bias(x3_v)

[x1_te, x2_te, x3_te, y_test] = splitData(data,11000,12000,'Task3.3')
x1test = bias(x1_te)
x2test = bias(x2_te)
x3test = bias(x3_te)
...
task33(x1training,x1validation,x1test,y_training)
print('Errors for set with top 60 words:')
task33(x2training,x2validation,x2test,y_training)
print('Errors for set with top 160 words:')
task33(x3training,x3validation,x3test,y_training)
